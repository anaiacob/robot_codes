...
public class Web_blue {
    ...
    public class Pipeline extends OpenCvPipeline {
        private final Scalar TARGET_COLOR = new Scalar(255, 0, 0);
        ...
        private String maxBlueROILabel = "Unknown";
        private int bluePixelCount = 0;
        ...
        public Mat processFrame(Mat input) {
            ...
            //  white pixels in ROI
            int[] blueCount = new int[3];

            for (int i = 0; i < 3; i++) {
                Rect roi = getRegionOfInterest(i, input.cols(), input.rows());
                Mat roiMask = new Mat(mask, roi);
                blueCount[i] = Core.countNonZero(roiMask);
            }


            // ROI with the max blue
            int maxBlueROI = getMaxBlueROI(blueCount);

            maxBlueROILabel = getROILabel(maxBlueROI);
            bluePixelCount = blueCount[maxBlueROI];

            telemetry.addData("Max Blue ROI", maxBlueROILabel);
            telemetry.addData("Blue Pixel Count", bluePixelCount);

            return input; // return original frame
        }


        public String getMaxBlueROILabel() {
            return maxBlueROILabel;
        }

        public int getBluePixelCount() {
        ...
        private int getMaxBlueROI(int[] blueCount) {
            // max index blueCount array
            int maxIndex = 0;
            for (int i = 1; i < blueCount.length; i++) {
                if (blueCount[i] > blueCount[maxIndex]) {
                    maxIndex = i;
                }
            }
            return maxIndex;
